...
Всё, что меняет состояние и что нельзя потерять, расходится доменными
событиями через topic-обменник RabbitMQ. События тонкие: несут идентификаторы,
а не снимки — детали потребитель дозапрашивает сам.
...
NATS несёт широковещательные обновления, где сообщение
...
Три схемы выше — чтобы понять. Эта — чтобы увидеть, во что превращаются
девять сервисов, четыре транспорта и семь баз, когда смотришь на них
одновременно. Смысл ровно один: столько связей приходится держать в голове,
если не разделять контуры.
...
Сплошная стрелка — REST, пунктир — gRPC и отбраковка, жирная — события
RabbitMQ, линия без стрелки — своя база. У `gateway` и `notification-service`
базы нет: первый ничего не хранит, второй держит состояние в очередях.

Разглядывать это не нужно — нужно заметить, что каждый транспорт по
отдельности читается за десять секунд, а вместе они не читаются вообще.
Поэтому выше их три, а не одна.
